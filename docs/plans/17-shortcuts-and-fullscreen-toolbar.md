...
## 상태
**조사 완료 · 구현 대기** (2026-09-15). Ian이 요청한 4건을 코드 조사와 실측으로 검토했다. 4건 모두
구현할 수 있다. 그중 ⌘B는 소스 편집기의 기존 "굵게" 단축키와 충돌한다. Ian의 결정은 **포커스로
구분**이다. 편집기에 커서가 있으면 굵게, 그 외에는 좌측 패널 토글로 동작한다.
