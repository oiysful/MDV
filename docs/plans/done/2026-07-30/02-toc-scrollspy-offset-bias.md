...
## 상태
완료 (2026-07-30) — `markdown.js`의 오프셋 계산을 `#scroll-area` 상대 좌표로 변경하고 리드값을 `-24`로 조정, 분할뷰 `#content` 스크롤 리스너 추가(a381613). 코드 리뷰로 분할/소스 모드 전환 시 오프셋이 재계산되지 않던 추가 결함(HIGH)을 발견해 `applySourceMode()`에서 재계산하도록 수정하고, 사이드바 접힘 애니메이션(.25s)이 끝난 뒤 한 번 더 재계산하도록 보강(c40153f, 316f81b). [`docs/self-check-request.md`](../2026-08-05/self-check-request.md) 2번 항목, 2026-07-22 요청.
