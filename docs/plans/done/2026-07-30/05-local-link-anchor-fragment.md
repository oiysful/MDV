...
## 상태
완료 (2026-07-30) — `openLocalLink` 진입부에서 `splitHrefFragment(href)`로 첫 `#` 기준 분리(5502a67). 코드 리뷰로 "파일명에 리터럴 `#`이 있는 경우 여전히 깨진다"는 계획 문서 자체의 리스크 노트를 정확히 반영하도록 관련 주석을 수정하고, `openLocalLink` 레벨에서 그 알려진 한계를 못박는 회귀 테스트를 추가(e7c52e5). [`docs/self-check-request.md`](../../../self-check-request.md) 5번 항목, 2026-07-22 요청.
