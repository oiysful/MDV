...
## 상태
완료 (2026-07-30) — `setActiveFilePath` 추가 및 `onTabsChanged` 연결(bb7700f), 이후 코드 리뷰로 `loadDir`의 `isRoot` 분기 밖으로 재적용 위치를 옮겨 하위 폴더 펼침 시에도 하이라이트가 반영되도록 보강(e7c52e5). [`docs/self-check-request.md`](../2026-08-05/self-check-request.md) 1번 항목, 2026-07-22 요청.
