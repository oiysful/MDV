...
`--test-concurrency=2` 전체 스위트를 4회 돌려 **4회 모두 104/104**였고, CI에서도 PR #15의
커밋 `47bef70`에 대해 `test-electron`을 3회 돌려(최초 실행 + `gh run rerun` 2회 — 같은
커밋이어야 표본이 된다) **3회 모두 성공**했다. 계획 18이 세운 기준을 그대로 적용한 것이다.

다만 무게는 위의 파괴 검증에 있다 — 연속 그린은 1/3 빈도에 대해 그 자체로 결정적이지 않고,
결정적인 것은 실패를 만들어내는 조건이 더 이상 실패를 만들지 못한다는 쪽이다.
