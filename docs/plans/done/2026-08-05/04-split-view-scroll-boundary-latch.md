...
## 상태
**원인 확정 및 수정 완료, 실사용 재확인 대기** (2026-08-04) — 2026-07-30에 "네이티브 브라우저 래칭"으로 잠정 결론 냈던 것을 Ian이 직접 재현 실험 없이 실사용 중 재현(한쪽으로 스크롤하다 바로 반대 방향으로 스크롤하면 무반응)해 **그 결론을 뒤집었다** — 앱 코드의 실제 결함이었다. 아래 "확정된 원인" 절 참고. 근본 수정을 `src/renderer/editor.js`에 적용 완료, 유닛 테스트 추가 완료 ([`docs/self-check-request.md`](../self-check-request.md) 4번 항목, 요청 2026-07-22).
...
## 리스크 / 미결정 사항
- **실사용 재확인 필요**: 이번 수정은 코드 감사와 유닛 테스트로 검증한 논리적 결함이며, 실제 트랙패드/휠 조작으로 재현·해소를 확인한 것은 아니다. Ian이 원래 보고한 조건(한쪽 스크롤 후 즉시 반대 방향 스크롤)으로 재확인되기 전까지는 "완료"가 아니라 "수정 적용, 확인 대기"로 남겨둔다.
- 이번에 고친 것은 에코 판별 로직의 버스트 상황 오판 버그이지, 브라우저/OS 레벨 오버스크롤 체이닝과는 별개다. 만약 재확인 후에도 유사한 증상이 남아 있다면 원인이 하나가 아니었을 가능성이 있다 — 그 경우 아래 `overscroll-behavior: contain` 후보를 재검토.
- **실측 후 재검토 후보**: `#content`/`#source-view`에 `overscroll-behavior: contain` 추가 — 실제 기기에서 체감 증상이 남아 있을 때만, 측정한 다음에 반영한다(측정 없이 넣으면 효과 없는 CSS가 남는다).
