...
## 상태
**구현·검증 완료** (2026-08-05) — 설계(2026-08-04) 그대로 4개 파일(`markdown.js`/`index.html`/`app-runtime.js`/두 테스트 파일)을 수정. 유닛 테스트 123개, Electron 스모크 테스트 76개(신규 긴 줄 겹침 회귀 테스트 포함) 전부 통과. 실제 앱을 띄워 라이트/다크 테마, hover 전/후, 긴 줄 스크롤 전/후 스크린샷으로 겹침 없음과 아이콘 클리핑 해소를 확인했다.
