...
## 상태
완료 (2026-07-20, 커밋 `33c6c02`) — 상세는 [`../plans/README.md`](./README.md#구현-요약-2026-07-20) 참고.
...
- README(`docs/plans/done/2026-07-20/README.md`)가 이미 "순수 정리 작업이라 급하지 않지만, IPC를 손댈 일이 생기면 같이 처리할 것"이라고 우선순위를 낮게 매겨뒀다 — 이 문서도 같은 판단을 유지한다. 별도로 시간 내서 할 만큼 급하지 않고, 다음에 `main.js`/`preload.js`/렌더러 IPC 호출부 중 하나라도 다른 이유로 건드릴 일이 생기면 그 김에 기계적으로 같이 처리하는 편이 리뷰 비용이 낮다.
