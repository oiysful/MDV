...
## 상태
**완료 (2026-09-23).** 조사 → 권고 1·2·3 반영(2026-09-21, PR #17) → 권고 4·5·6 + §4 판단
결정(2026-09-23, Ian — 아래 "결정" 절) → 결정이 남긴 추가 작업 두 건 반영(PR #19).

권고 5가 남긴 두 건이 실제로 들어간 모습은 이렇다. `tests/unit/theme.test.js` 16건이
`createThemeController`를 순수 스텁으로 덮고(스텁이 최종값이 아니라 **호출을 기록**한다 —
고정한 동작 중 둘이 *부재*이기 때문이다), `source-mode-toc.test.js:135`가 두 `##` 헤딩 사이
지점을 본다. 여섯 건의 의도적 회귀로 둘 다 반증 확인했다. **Electron 테스트는 하나도 내리지
않았다** — 그게 이 결정의 핵심이었다.

곁다리 수확: 그 커버리지를 쓰다가 `theme.js`의 실제 결함이 드러났다(알 수 없는 저장값 하나로
테마 버튼이 영구히 죽는다). 이슈 #20 → PR #21로 별도 처리했다.
