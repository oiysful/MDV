...
**단 하나가 열려 있다 — 실기 확인.** 확인된 것은 `/Volumes` 경로가 kqueue 백엔드를 타지 않고
폴링으로 실제 변경이 배달된다는 것까지다. 진짜 vnode 리보크(외장 디스크 꺼내기, 공유 끊김,
슬립-웨이크)에서 살아남는 것은 재현 장비 없이 검증할 수 없다. 7절의 "실기 확인(대체 불가)"
항목이며, 그것이 "abort 경로를 타지 않는다"를 "크래시가 막혔다"로 바꾸는 유일한 단계다.
