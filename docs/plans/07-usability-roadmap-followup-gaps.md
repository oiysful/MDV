...
## 상태
A/C/D는 조사 완료, 구현 대기 (2026-08-04). B(검색)는 최초 조사에서 "이미 해결되어 조치 불필요"로 잘못 결론 냈다가, 같은 날 Ian의 직접 보고로 정정 후 [`08-search-highlight-and-ime-fixes.md`](./08-search-highlight-and-ime-fixes.md)에서 별도로 완료 — 삭제된 로컬 스크래치 `.sisyphus/plans/usability-feature-roadmap.md`(2026-07-13 SHIPPED, `AGENTS.md:124`에 따라 추적 대상 문서가 아니었음)가 "Known gaps left behind"로 남겨둔 4건을 코드 조사로 재검증한 결과다.
...
- 근본 원인 분석과 수정은 [`docs/plans/08-search-highlight-and-ime-fixes.md`](./08-search-highlight-and-ime-fixes.md)에 기록했다(완료). **교훈**: "라우팅이 맞다"는 "최종 시각적 결과가 맞다"를 보장하지 않는다 — 코드 감사만으로 UI 버그의 부재를 단정하지 말 것.
...
- `src/renderer/app-shell.js` 또는 `app-runtime.js` — 구분선 드래그 핸들러(A).
- `src/renderer/app-runtime.js` — `copyAll`/`copyCode` 클립보드 실패 처리(C).
- `src/renderer/explorer.js` — 빈 상태 힌트 문구 수정(D).

## 테스트 계획
- A: `tests/electron` 스모크에 분할뷰 진입 → 구분선 드래그 → 두 패널 폭이 변경되는지 확인하는 케이스 추가. 폭 비율을 세션에 저장하기로 하면 `workspace.test.js`에 저장/복원 케이스도 추가.
- C: `tests/unit/app-runtime.test.js`에 `navigator.clipboard.writeText`를 reject하도록 스텁해 `showToast`가 실패 문구로 호출되는지 확인하는 케이스 추가(현재 스위트에 clipboard 관련 테스트 없음 — 신설 필요).
- D: `tests/unit/explorer.test.js`에 `EXPLORER_EMPTY_HTML` 문자열이 실제 버튼 라벨과 일치하는지(또는 스냅샷) 확인하는 케이스 추가.

## 리스크 / 미결정 사항
- A: 구분선 위치를 세션별로 영구 저장할지, 세션 내에서만 유지할지는 제품 결정이 필요하다 — 범위를 좁혀 "세션 내 유지만" 먼저 구현하고 영구 저장은 v2로 미루는 것을 권장.
- C: 클립보드 실패 시 토스트 문구("복사 실패" 등)의 정확한 카피는 미정 — 기존 토스트 톤(`복사됨`, `코드 복사됨`)과 일관되게 정할 것.
- D: 이 항목은 사소하지만 다른 세 항목과 함께 처리하면 리뷰 비용이 낮다.
