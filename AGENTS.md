...
- Two Electron tests are **flaky on CI only, and lowering concurrency did not fix them — it only made them rarer**. `large-directory-watch.test.js`'s "a pathologically large directory trips the path-count guard instead of hanging the app" fails as a `TimeoutError`, and `split-view-async.test.js`'s "split view restores fresh preview and pane scroll after immediate tab switch" fails as an `AssertionError`. Neither reproduces locally. This exact pair is what the `--test-concurrency` comment in `ci-electron.yml` describes, and that comment reads as though dropping 4 → 2 settled it — it did not: both failed again at concurrency 2 on PR #8 (run 35202862668, 2026-09-17, 102/104 passing) and a plain re-run of the same commit went green. So **a red `test-electron` naming only these two is not automatically a regression** — re-run the job first, and only dig in if it fails twice or a third test joins them. The underlying fix is the one applied on 2026-08-06 for the same class of failure: replace fixed/implicit waits with explicit condition waits, rather than tuning concurrency again.
